...
export class StateManager {
  ...
  private subscribers: Array<
    (event: AppEvent, state: AppState, oldState: AppState) => void
  > = [];
  ...
  publishEvent(event: AppEvent) {
    ...
    try {
      ...
      nextState = eventHandlers
        .find((x) => x.eventType === event.type)
        ?.handler(event as any, this.appState);
    } catch (e) {
  ...
  subscribe(
    fn: (event: AppEvent, state: AppState, oldState: AppState) => void,
  ) {
